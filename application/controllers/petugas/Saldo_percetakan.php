...
class Saldo_percetakan extends MY_Controller
{
	...
	public function riwayat()
	{
		// pagination
		$start = $this->uri->segment(4, 0);
		$config['base_url'] = base_url() . 'petugas/saldo/riwayat/';
		// Class bootstrap pagination yang digunakan
		$config['full_tag_open'] = "<ul class='pagination pagination-sm no-margin pull-right'>";
		$config['full_tag_close'] = "</ul>";
		$config['num_tag_open'] = '<li>';
		...
		$this->generateCsrf(); 
		$this->render('petugas/saldo/riwayat', $data);
	}

	public function tarik()
	{
		$data['nominal'] = str_replace(".", "", $this->input->post('nominal'));
		$data['id_percetakan'] = $this->petugas_model->getPercetakan();
		$data['status'] = '5';
		$data['keterangan'] = 'Penarikan saldo sebesar Rp.'.$this->input->post('nominal');
		$this->saldo_percetakan_model->insert($data);

		$this->message('Permintaan penarikan berhasil dikirim!', 'success');
		$this->go('petugas/saldo_percetakan');
	}
