...
class Auth extends MY_Controller {
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				if ($user->group_id === "1") {
					...
					if (isset($back_url)) {
						redirect('percetakan/detail/'.$back_url, 'refresh');
					} else {
			...
			else
			{
				...
				$data['password'] = array('name' => 'password',
					...
					if (isset($url)) {
						redirect('percetakan/detail/'.$back_url.'/#login', 'refresh');
					} else {
						redirect('auth/login', 'refresh');
					}
			}
	...
	public function activate($id, $code=false)
	{
		...
		else
		{
			...
			$this->message($this->ion_auth->errors());
			dump($this->message($this->ion_auth->errors()));
			// redirect("auth/forgot_password", 'refresh');
		}
	}
	...
	public function create_user()
	{
		...
		else
		{
			...
			$this->render('auth/pendaftaran', $this->data);
		}
	}

	public function register_success()
	{
		$this->render('auth/pendaftaran_berhasil');
	}
