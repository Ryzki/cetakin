...
@section('title')Detail Percetakan
@endsection
  ...
  <div class="container">
    <div class="row">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <table class="table table-sm" style="font-size: 15px">
              ...
              <?php else: ?> @foreach($info_harga as $harga)
              <tr>
                <td>{{$harga->nama}}</td>
                <td><b>{{rupiah($harga->harga)}} /{{$harga->satuan}}</b></td>
              </tr>
              @endforeach
              <?php endif ?>
            </table>
      ...
      <div class="col-md-5">
        <div class="card">
          <div class="card-header bg-primary">
            <i class="fa fa-bullhorn"></i> Ayo Mulai Cetak Foto di {{$data->nama}}!
          </div>
          <div class="card-body">
            <div class="row">
            ...
            <form action="{{site_url('percetakan/cetak_foto')}}" method="post" enctype="multipart/form-data">
              {{$csrf}} {{form_hidden('url', $data->id.'/'.$data->slug);}} {{form_hidden('idpercetakan', $data->id);}}
 
              <div class="form-group">
                <label for="ukuran">Ukuran (3x4, 4R, 5R, dll.)</label>
                <input name="ukuran" type="text" class="form-control" id="ukuran" value="{{set_value('ukuran')}}">
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                <label for="file">File Cetak (.png, .jpg, .svg)</label>
                <input type="file" name="file" class="form-control-file" id="file">
              </div>
              <div class="form-group">
                <label for="catatan">Catatan</label>
                <textarea name="catatan" id="catatan" class="form-control" rows="7" cols="90" placeholder="Masukan Detail mengenai cetakan kamu">{{set_value('catatan')}}</textarea>
              </div>
              <div align="right">
      ...
      <div class="col-md-5" id="login">
        <div class="card">
          <div class="card-header bg-primary">
            <i class="fa fa-bullhorn"></i> <b>Ayo Mulai Cetak Foto di {{$data->nama}}!</b>
          </div>
          <div class="card-body">
            ...
            <form method="post" action="{{site_url('auth/login_pelanggan')}}">
              <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" />              {{form_hidden('url', $data->id.'/'.$this->uri->segment(4).'/'.$data->slug);}}
              ...
              <br><br> Belum memiliki akun? <a href="{{site_url('pendaftaran')}}" class="btn btn-secondary"><i class="fa fa-pencil-square-o"></i> Daftar Sekarang!</a>
            </form>
          </div>
        </div>
      </div>
