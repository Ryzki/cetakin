    ...
    <section class="content">
      ...
      <a href="{{site_url('admin/petugas')}}" class="btn btn-info"><i class="fa fa-arrow-left"></i> kembali</a>
        ...
        <div class="box-body">
          ...
          <form class="form-horizontal" action="{{site_url('admin/petugas/save')}}" method="post">
          {{$csrf}}
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <select name="idusers" class="form-control" id="idusers">
                  ...
                  <option {{(set_value('idusers') == $datauser->id)?'selected':' '}} value="{{$datauser->id}}">{{$datauser->first_name}}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <select name="idpercetakan" class="form-control" id="idpercetakan">
                  ...
                  <option {{(set_value('idpercetakan') == $datapercetakan->id)?'selected':' '}} value="{{$datapercetakan->id}}">{{$datapercetakan->nama}}</option>
                  @endforeach
                </select>
              </div>
            </div>
